...
{
	INucleus* pGalaxy = g_pWebRTImpl->GetNucleus(m_hWnd);
	if (pGalaxy)
	{
		IXobj* pObj = nullptr;
	...
	CWebPageImpl* pImpl = g_pWebRTImpl->GetWebPageImpl(m_hWnd);
	CBrowserImpl* pBrowser = g_pWebRTImpl->GetBrowserImpl(m_hWnd);
