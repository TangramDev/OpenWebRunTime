...
namespace CommonUniverse
{
	...
	public:
		...
		{
			if (g_pWebRTImpl && g_pWebRTImpl->m_hMainWnd && ::IsWindow(g_pWebRTImpl->m_hMainWnd) == false)
				...
				if (pWnd)
				{
					...
					if (pWnd->Create(nullptr, _T(""), WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0), pParent, 0, nullptr))
					{
						...
						if (pWnd->IsKindOf(RUNTIME_CLASS(CView)))
						{
							...
							if (pFrame)
							{
								pWebRTFrameWndInfo = (WebRTFrameWndInfo*)::GetProp(pFrame->m_hWnd, _T("WebRTFrameWndInfo"));
	...
	bool CWebRTApp::WebRTInit(CString strID)
	{
		...
		if (hModule)
		{
			...
			if (!afxContextIsDLL)
			{
				...
				g_pWebRTImpl->InserttoDataMap(0, m_strProviderID, static_cast<IUniverseAppProxy*>(this));
				...
				if (g_pWebRTImpl->m_nAppType != APP_BROWSER &&
					...
					::PostAppMessage(::GetCurrentThreadId(), WM_CHROMEAPPINIT, (WPARAM)m_pWebRTImpl->m_pWebRTDelegate, g_pWebRTImpl->m_nAppType);
			...
			if (pWnd && pWnd->IsKindOf(RUNTIME_CLASS(CView)))
			{
				...
				if (pDoc)
				{
					...
					if (pFrame)
					{
						...
						bool bRet = g_pWebRTImpl->SetFrameInfo(hWnd, hRetFrame, strInfo, pDoc, pTemplate);
						if (pWnd->IsKindOf(RUNTIME_CLASS(CScrollView)) && ::GetParent(hWnd) == hRetFrame)
							((CScrollView*)pWnd)->ResizeParentToFit();
						pWebRTFrameWndInfo = (WebRTFrameWndInfo*)::GetProp(hRetFrame, _T("WebRTFrameWndInfo"));
	...
	bool CWebRTApp::SetFrameCaption(HWND hWnd, CString strCaption, CString strAppName)
	{
		...
		if (pWnd)
		{
			...
			if (pFrame)
			{
				WebRTFrameWndInfo* pWebRTFrameWndInfo = (WebRTFrameWndInfo*)::GetProp(pFrame->m_hWnd, _T("WebRTFrameWndInfo"));
				if (pWebRTFrameWndInfo)
				{
					...
					if (pDoc)
					{
						...
						if (strPath == _T(""))
						{
							...
							CDocTemplate* pTemplate = (CDocTemplate*)pWebRTFrameWndInfo->m_pDocTemplate;
							CString name = _T("");
							...
							case 1:
							case 4:
								::SetWindowText(pFrame->m_hWnd, strTitle + _T(" - ") + strAppName + _T(" - ") + g_pWebRTImpl->m_strAppName);
								break;
							default:
	...
	CString CWebRTApp::QueryParentInfo(HWND hPWnd, void* lpInfo)
	{
		...
		if (pParent)
		{
			...
			if (pParentFrame)
			{
				...
				CDocTemplate* pTemplate = pDoc->GetDocTemplate();// pContext->m_pNewDocTemplate;
				CString strExt = _T("");
	...
	void CWebRTApp::OpenDocFile(CString strFileName, CString strExt, CString strCreatingDOCID)
	{
		POSITION nPos = GetFirstDocTemplatePosition();
		while (nPos)
		{
	...
	void CWebRTApp::OnIPCMsg(CWebPageImpl* pWebPageImpl, CString strType, CString strParam1, CString strParam2, CString strParam3, CString strParam4, CString strParam5)
	{
		if (strType.CompareNoCase(_T("COSMOS_CREATE_DOC")) == 0)
		{
			...
			if (pTemplate && m_strCreatingDOCID == _T("") && nCount == 1)// && strParam1.CompareNoCase(_T(".abc")) == 0)
			{
				m_strCreatingDOCID = strParam2;
				pTemplate->OpenDocumentFile(nullptr);
				return;
	...
	HWND CWebRTAppEx::QueryWndInfo(QueryType nType, HWND hWnd)
	{
		...
		case CanClose:
		{
			...
			//					m_pMainWnd = pFrame;
			//					g_pWebRTImpl->m_hMainWnd = m_pMainWnd->m_hWnd;
			//					return pFrame->m_hWnd;
			//				}
			//			}
			//		}
				...
				if (pDoc)
				{
					...
					if (pFrame)
					{
						...
						if (it == m_mapDOMObj.end())
							m_mapDOMObj[strName] = pClsInfo;
						bool bRet = g_pWebRTImpl->SetFrameInfo(hWnd, hRetFrame, strInfo, pDoc, pTemplate);
						if (pWnd->IsKindOf(RUNTIME_CLASS(CFormView)) && ::GetParent(hWnd) == hRetFrame)
	...
	bool CWebRTAppEx::SetFrameCaption(HWND hWnd, CString strCaption, CString strAppName)
	{
		if (strCaption == _T("") || !::IsWindow(hWnd))
			return false;
		CWnd* pWnd = CWnd::FromHandlePermanent(hWnd);
	...
	void CWebRTAppEx::OpenDocFile(CString strFileName, CString strExt, CString strCreatingDOCID)
	{
		POSITION nPos = GetFirstDocTemplatePosition();
		while (nPos)
		{
	...
	void CWebRTAppEx::OnIPCMsg(CWebPageImpl* pWebPageImpl, CString strType, CString strParam1, CString strParam2, CString strParam3, CString strParam4, CString strParam5)
	{
		if (strType.CompareNoCase(_T("COSMOS_CREATE_DOC")) == 0)
		{
			int nCount = 0;
	...
	LRESULT CWebRTMDIFrame::OnQueryAppProxy(WPARAM wp, LPARAM lp)
	{
		if (lp)
		{
			switch (lp)
	...
	BOOL CWebRTMDIFrame::OnNcActivate(BOOL bActive)
	{
	...
	void CWebRTMDIFrame::OnClrControlCreated(IXobj* Node, IDispatch* Ctrl, CString CtrlName, HWND CtrlHandle)
	{
		CComBSTR bstrName("");
