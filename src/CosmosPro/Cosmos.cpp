...
namespace Universe
{
	...
	Xobj::Xobj(IXobj* pXobj)
	{
		...
		if (theApp.m_pWebRTImpl)
			theApp.m_pWebRTImpl->AttachXobj(m_pXobjEvent);
	...
	int WebRT::WebRTInit(String^ strInit)
	{
		...
		if (strInfo != _T(""))
		{
			...
			if (nPointer)
			{
				theApp.m_pWebRTImpl = (CWebRTImpl*)nPointer;
				theApp.m_pWebRTImpl->m_pCLRProxy = &theAppProxy;
				theApp.m_pWebRTImpl->m_pWebRTAppProxy = static_cast<IUniverseAppProxy*>(&theApp);
				//20180817
	...
	void WebRT::Run()
	{
		...
		if (theApp.m_pWebRTImpl)
		{
			theApp.m_pWebRTImpl->Run();
		}
		System::Windows::Forms::Application::Exit();
	}

	void WebRT::Run(Form^ Mainform)
	{
		if (WebRT::WebRuntimeInit())
			return;
		if (theApp.m_pWebRTImpl)
		{
			WebRT::MainForm::set(Mainform);
			theApp.m_pWebRTImpl->Run();
		}
		Forms::Application::Exit();
	}

	void WebRT::Run(ApplicationContext^ context)
	{
		if (WebRT::WebRuntimeInit())
			return;
		if (theApp.m_pWebRTImpl)
		...
		if (theApp.m_pWebRT == nullptr)
		{
			...
			if (hModule == nullptr) {
				...
				if (SHGetFolderPath(NULL, CSIDL_PROGRAM_FILES, NULL, 0, m_szBuffer) ==
					...
					m_strProgramFilePath += _T("\\WebRT\\universe.dll");
			...
			if (hModule) {
				...
				if (_pWebRTImplFunction != NULL) {
					...
					if (theApp.m_pWebRTImpl->m_nAppType == 0)
						theApp.m_pWebRTImpl->m_nAppType = APP_BROWSERAPP;
					if (theApp.m_pWebRTImpl->m_pWebRTDelegate == nullptr)
					{
	...
	WebRT^ WebRT::InitWebRTApp(bool bSupportCrashReporting, CosmosAppType AppType)
	{
		...
		if (theApp.m_bBrowserModeInit)
			return m_pWebRT;
		theApp.m_pWebRTImpl->m_nAppType = (DWORD)AppType;
		//m_nAppType = AppType;
		switch (AppType)
		{
	...
	{
		return theApp.m_pWebRTImpl->m_bIsSupportCrashReporting;
	}

	void WebRT::SupportCrashReporting::set(bool bSupportCrashReporting)
	{
		if (theApp.m_pWebRTImpl->m_bIsSupportCrashReporting == false)
			theApp.m_pWebRTImpl->m_bIsSupportCrashReporting = bSupportCrashReporting;
	}

	CosmosAppType WebRT::AppType::get()
	{
		DWORD _nType = theApp.m_pWebRTImpl->m_nAppType;
		switch (_nType)
		{
		case APP_WIN32:
	...
	void WebRT::AppType::set(CosmosAppType nType)
	{
		if (m_pWebRT == nullptr)
			m_pWebRT = gcnew WebRT();
		theApp.m_pWebRTImpl->m_nAppType = (DWORD)nType;
	}
	...
	void WebRT::MainForm::set(Form^ frm)
	{
		...
		theApp.m_pWebRTImpl->SetMainWnd((HWND)frm->Handle.ToPointer());
	}
	...
	String^ WebRT::NTPXml::get()
	{
		BSTR bstrNtpXml = theApp.m_pWebRTImpl->m_strNtpXml.AllocSysString();
		String^ strResult = marshal_as<String^>(bstrNtpXml);
		::SysFreeString(bstrNtpXml);
		return strResult;
	}

	void WebRT::NTPXml::set(String^ strXml)
	{
		CString _strXml = strXml;
		CTangramXmlParse m_Parse;
		if (m_Parse.LoadXml(_strXml))
		{
			CTangramXmlParse* pParse = m_Parse.GetChild(TGM_NUCLEUS);
			if (pParse)
			{
				...
				if (nCount == 1)
				{
					...
					if (pParse)
					{
						theApp.m_pWebRTImpl->m_strNtpXml = _strXml;
					}
				}
			}
		}
	...
	{
		auto it = theApp.m_pWebRTImpl->m_mapValInfo.find(marshal_as<CString>(iIndex));
		if (it != theApp.m_pWebRTImpl->m_mapValInfo.end())
		{
	...
	Xobj^ WebRT::GetXobjFromControl(Control^ ctrl)
	{
		...
		auto it = theApp.m_pWebRTImpl->m_mapXobj.find(hCtrl);
	...
	void WebRT::UpdateNewTabPageLayout(String^ newTabPageLayout)
	{
		WebRT::NTPXml::set(newTabPageLayout);
	}
	...
	{
		BSTR bstrAppDataPath = theApp.m_pWebRTImpl->m_strAppDataPath.AllocSysString();
			...
			if (pApp->m_pWebRTCLRTypeDic->TryGetValue(m_strID, pType) == false)
			{
				...
				if (nIndex != -1)
				{
					...
					if (s1 == _T("host"))
					{
						...
						_strID = _strID.Left(nIndex + 1) + theApp.m_pWebRTImpl->m_strExeName;
						_strID.MakeLower();
					}
				}
	...
	Browser^ WebRT::CreateBrowser(IntPtr ParentHandle, String^ strUrls)
	{
		...
		if (theApp.m_pWebRTImpl->m_pBrowserFactory)
		{
			strUrls += L"|";
			strUrls = strUrls->Replace(L"||", L"|");
			CString strPath = theApp.m_pWebRTImpl->m_strAppPath;

			strUrls = strUrls->Replace(L"host:", marshal_as<String^>(strPath));
			if (ParentHandle == (IntPtr)1)
			{
				hPWnd = theApp.m_pWebRTImpl->m_hCosmosWnd;
			}

			HWND hWnd = theApp.m_pWebRTImpl->m_pBrowserFactory->CreateBrowser(hPWnd, strUrls);
	...
	Universe::Wormhole^ Universe::Xobj::Wormhole::get()
	{
		if (m_pXobj)
		{
			...
			{
				CSession* pSession = theApp.m_pWebRTImpl->GetCloudSession(m_pXobj);
				if (pSession)
				{
	...
	{
		HWND hWnd = (HWND)ctrl->Handle.ToPointer();
		auto it = theApp.m_pWebRTImpl->m_mapCtrlTag.find(hWnd);
		if (it != theApp.m_pWebRTImpl->m_mapCtrlTag.end())
			return marshal_as<String^>(it->second);
		return L"";
	}
	...
	void WinNucleus::SendMessage(String^ strFrom, String^ strTo, String^ strMsgId, String^ strMsgContent, String^ strExtra)
	{
		if (m_pNucleus)
		{
			...
			theApp.m_pWebRTImpl->m_pCurrentIPCMsg = &msg;
			::SendMessage(hWnd, WM_HUBBLE_DATA, (WPARAM)&msg, 20200203);
		}
		theApp.m_pWebRTImpl->m_pCurrentIPCMsg = nullptr;
	}
