	...
	if (::GetModuleHandle(_T("universe.dll")) == nullptr)
	{
		...
		if (hModule) {
			...
			if (_pWebRTImplFunction != NULL) {
				theApp.m_pWebRTImpl = _pWebRTImplFunction(&theApp.m_pWebRT);
				theApp.m_pWebRTImpl->m_pWebRTDelegate = (IWebRTDelegate*)&theApp;
				theApp.m_pWebRTImpl->m_pUniverseAppProxy = (IUniverseAppProxy*)&theApp;
			}
		}
	}
...
CWebRTProxy::CWebRTProxy() : IWebRTCLRImpl()
{
	...
	if (::GetModuleHandle(_T("universe.dll")) == nullptr)
	{
		...
		if (theApp.m_pWebRT)
		{
			theApp.m_pWebRTImpl->m_pCLRProxy = this;
			theApp.m_pWebRTImpl->m_pWebRTAppProxy = &theApp;
		}
	}
...
CWebRTProxy::~CWebRTProxy()
{
	...
	if (theApp.m_bHostApp == false)
		theApp.m_pWebRTImpl->m_pCLRProxy = nullptr;
	OutputDebugString(_T("------------------Release CWebRTProxy ------------------------\n"));
	CString strInfo = _T("");
}

bool CWebRT::DoIdleWork() {
	Universe::WebRT::GetWebRT()->Fire_OnCloudAppIdle();
	return false;
}

HWND CWebRT::InitWebRTApp()
{
	return 0;
};

void CWebRT::IPCMsg(HWND hWnd, CString strType, CString strParam1, CString strParam2)
{
			...
			if (pPForm != nullptr)
			{
				if (IsWinForm(hPWnd))
				{
					...
					if (it == m_mapForm.end())
					{
						...
						::SendMessage(theApp.m_pWebRTImpl->m_hCosmosWnd, WM_WINFORMCREATED, (WPARAM)hPWnd, (LPARAM)0);
					}
				}
			}
		}
		if (hPWnd == NULL || (::GetWindowLong(hWnd, GWL_EXSTYLE) & WS_EX_MDICHILD))
		{
			if (_strClassName.Find(_T("WindowsForms10.Window")) == 0)
			{
				::PostAppMessage(theApp.m_dwThreadID, WM_COSMOSMSG, (WPARAM)hWnd, 20210913);
			}
		}
	}
}
...
void CWebRTProxy::WindowDestroy(HWND hWnd)
{
	...
	auto it7 = theApp.m_pWebRTImpl->m_mapUIData.find(hWnd);
	if (it7 != theApp.m_pWebRTImpl->m_mapUIData.end())
	{
		theApp.m_pWebRTImpl->m_mapUIData.erase(it7);
	}
...
void CWebRTProxy::OnVisibleChanged(System::Object^ sender, System::EventArgs^ e)
{
	...
	if (pChild->Visible)
	{
			...
			if (pForm)
			{
				...
				if ((theAppProxy.m_strExtendableTypes->IndexOf(L"|" + strType2 + L"|") != -1 && pChild->Dock == DockStyle::None) || pChild->Dock == DockStyle::Fill)
				{
					...
					if (bExtendable)
					{
						...
						theApp.m_pWebRTImpl->ConnectNuclei(NULL, (HWND)pChild->Handle.ToInt64(), strName, pNuclei->m_pNuclei, pInfo);
					}
				}
		...
		INuclei* pXobj = theApp.m_pWebRTImpl->Observe((HWND)pChild->Handle.ToInt64(), marshal_as<CString>(pChild->Name), _T("default"));
	}
}
				...
				if (strTag->IndexOf(L"|TangramNode|") != -1)
				{
					...
					if (String::IsNullOrEmpty(strIndex) == false)
					{
						...
						if (pType->IsSubclassOf(Form::typeid))
						{
							...
							theApp.m_pWebRTImpl->ObserveCtrl(pCtrl->Handle.ToInt64(), name, strIndex);
						}
						pCtrl->Select();
					}
				}
					...
					if ((m_strExtendableTypes->IndexOf(L"|" + strType2 + L"|") != -1 && pChild->Dock == DockStyle::None) || pChild->Dock == DockStyle::Fill)
					{
						...
						if (bExtendable)
						{
							...
							if (strType == L"System.Windows.Forms.TreeView")
							{
								...
								if (_pChild)
								{
									...
									if (_pChild2)
									{
										...
										theApp.m_pWebRTImpl->m_mapUIData[(HWND)pChild->Handle.ToPointer()] = _pChild2->xml();
							...
							else if (name->ToLower() == L"mdiclient")
							{
								...
								INucleus* _pGalaxy = theApp.m_pWebRTImpl->ConnectNuclei(hWnd, (HWND)pChild->Handle.ToInt64(), strName, pNuclei->m_pNuclei, pInfo);
				...
				if (bExtendable)
				{
					...
					if (pParse)
					{
						...
						if (pChildParse2)
						{
							...
							pInfo->m_strParentCtrlName = pCtrl->Name->ToLower();
							INucleus* _pGalaxy = theApp.m_pWebRTImpl->ConnectNuclei(NULL, (HWND)pChild->Handle.ToInt64(), strName, pNuclei->m_pNuclei, pInfo);
							if (m_pOnCtrlVisible)
							{
							}
							else
...
void CWebRTProxy::CtrlInit(int nType, Control^ ctrl, INuclei* pNuclei)
{
	...
	auto it = theApp.m_pWebRTImpl->m_mapUIData.find(hWnd);
	if (it != theApp.m_pWebRTImpl->m_mapUIData.end())
	{
		switch (nType)
		{
		case 0:
		...
		theApp.m_pWebRTImpl->m_mapUIData.erase(it);
	}
}

System::Void CWebRTProxy::LoadNode(TreeView^ pTreeView, TreeNode^ pXobj, INuclei* pNuclei, CTangramXmlParse* pParse)
{
	if (pParse)
	{
		...
		if (m_Parse.LoadXml(bstrObjID))
		{
			...
			if (strObjID != _T(""))
			{
				...
				if (pObj != nullptr)
				{
					if (pObj->GetType()->IsSubclassOf(Form::typeid))
					{
						...
						{
							theApp.m_pWebRTImpl->m_hMainWnd = (HWND)thisForm->Handle.ToPointer();
						}
						...
						{
							pWebRTSession = theApp.m_pWebRTImpl->CreateCloudSession(pProxyBase);
							pCloudSession = gcnew Wormhole(pWebRTSession);
							Universe::WebRT::Wormholes[pObj] = pCloudSession;
							pCloudSession->m_pHostObj = pObj;
							CString strFormName = m_Parse.attr(_T("formname"), _T(""));
			...
			else
			{
				if (Universe::WebRT::m_pMainForm)
				{
					...
					theApp.m_pWebRTImpl->m_hMainWnd = hWnd;
		...
		if (m_Parse.LoadXml(bstrObjID))
		{
			...
			if (strObjID != _T(""))
			{
				...
				if (nHandle)
				{
					...
					if (pWebPage != nullptr&&::IsWindow(hPage))
					{
						pPage = gcnew WebPage(pWebPage);
						pPage->m_hWnd = hPage;
						if (pChild)
				...
				bool bIsMainFormExists = false;
				Object^ pObj = nullptr;
				bool bMainForm = m_Parse.attrBool(_T("mainwindow"));
				if (pObj == nullptr && bMainForm && Universe::WebRT::m_pMainForm)
				{
					pObj = Universe::WebRT::m_pMainForm;
					Universe::WebRT::MainForm::set(Universe::WebRT::m_pMainForm);
					theApp.m_pWebRTImpl->m_hMainWnd = (HWND)Universe::WebRT::m_pMainForm->Handle.ToPointer();
				...
				if (pObj != nullptr)
				{
					if (pObj->GetType()->IsSubclassOf(Form::typeid))
					{
						...
						int nWidth = m_Parse.attrInt(_T("width"), 0);
						int nHeight = m_Parse.attrInt(_T("height"), 0);
						if (!bIsMainFormExists && nWidth * nHeight)
						{
							thisForm->Width = nWidth;
							thisForm->Height = nHeight;
						}
						...
						if (!bIsMainFormExists && strCaption != _T(""))
							thisForm->Text = marshal_as<String^>(strCaption);
						if (thisForm->IsMdiContainer)
						{
							Control^ mdiclient = Universe::WebRT::GetMDIClient(thisForm);
...
IDispatch* CWebRTProxy::CreateObject(BSTR bstrObjID, HWND hParent, IXobj* pHostNode, CWebPageImpl* pImpl, IWebPage* pPage)
{
	String^ strID = marshal_as<String^>(bstrObjID);
	Object^ _pObj = Universe::WebRT::CreateObject(strID);
	...
	if (_pObj->GetType()->IsSubclassOf(Control::typeid))
	{
		...
		if (pObj != nullptr && pHostNode)
		{
			...
			theApp.m_pWebRTImpl->m_mapXobj[hWnd] = pHostNode;
			...
			if (pObj->GetType()->IsSubclassOf(Form::typeid))
			{
				...
				if (m_Parse.LoadXml(strXml) || m_Parse.LoadFile(strXml))
				{
					...
					if (pChild3)
					{
						...
						if (nCount)
						{
							if (theApp.m_pWebRTImpl->m_pCurMDIChildFormInfo)
							{
								delete theApp.m_pWebRTImpl->m_pCurMDIChildFormInfo;
								theApp.m_pWebRTImpl->m_pCurMDIChildFormInfo = nullptr;
							}
							CMDIChildFormInfo* pInfo = new CMDIChildFormInfo();
							theApp.m_pWebRTImpl->m_pCurMDIChildFormInfo = pInfo;
							for (int i = 0; i < nCount; i++)
							{
				...
				if (it != m_mapForm.end())
					m_mapForm.erase(it);
				theApp.m_pWebRTImpl->m_hFormNodeWnd = hWnd;
			...
			//if (theApp.m_pWebRTImpl->IsMDIClientNucleusNode(pHostNode) == false && bProperty == false)
			{
...
void CWebRTProxy::ReleaseCosmosObj(IDispatch* pDisp)
{
	...
	if (pObj->GetType()->IsSubclassOf(Control::typeid))
	{
		...
		auto it = theApp.m_pWebRTImpl->m_mapXobj.find(hWnd);
		if (it != theApp.m_pWebRTImpl->m_mapXobj.end())
			theApp.m_pWebRTImpl->m_mapXobj.erase(it);
...
{
	CSession* pSession = theApp.m_pWebRTImpl->GetCloudSession(pXobj);
	if (pSession == nullptr)
		return;
...
void CWebRTProxy::ProcessFormWorkState(HWND hForm, int nState)
{
	...
	case 2:
	{
		...
		if (pCtrl != nullptr)
		{
			...
			CWebPageImpl* pProxy = theApp.m_pWebRTImpl->m_pMainWebPageImpl;
			if (pProxy)
			{
				pSession = theApp.m_pWebRTImpl->CreateCloudSession(pProxy);
			...
			if (form && form->IsMdiContainer)
			{
				...
				if (button->Tag != nullptr)
				{
					...
					if (String::IsNullOrEmpty(strXml) == false)
					{
						...
						IDispatch* pFormDisp = theApp.m_pWebRTImpl->m_pCLRProxy->CreateCLRObj(strXml);
						if (pFormDisp)
						{
							auto it = theAppProxy.m_mapToolStripButton.find(hForm);
							if (it != theAppProxy.m_mapToolStripButton.end())
...
void CWebRTProxy::CosmosAction(BSTR bstrXml, void* pvoid)
{
	...
	if (strXml != _T(""))
	{
		if (strXml.CompareNoCase(_T("startclrapp")) == 0)
		{
			if (m_bInitApp == false)
			{
				...
				case CosmosAppType::APPWIN32:
				{
					...
					if (Universe::WebRT::MainForm == nullptr || theApp.m_pWebRTImpl->m_hMainWnd == NULL)
					{
					}
				}
				break;
				...
				{
					theApp.m_pWebRTImpl->m_hMainWnd = NULL;
					Form^ mainForm = Universe::WebRT::MainForm::get();
					if (mainForm)
						theApp.m_pWebRTImpl->m_hMainWnd = (HWND)mainForm->Handle.ToPointer();
					theApp.InitWebRTApp(theApp.m_pWebRTImpl->m_bIsSupportCrashReporting);
				}
				break;
				case CosmosAppType::APPECLIPSE:
				{
					if (theApp.m_pWebRT && !theApp.m_pWebRTImpl->m_bIsEclipseInit)
					{
		...
		//		if (nPos != -1)
		//		{
		//			Universe::WebRT::Fire_OnWebRTIPCMessage(pWndXobj, marshal_as<String^>(strXml));
		//		}
		//		else
		//			Universe::WebRT::Fire_OnWebRTLoadDocument2Viewport(pWndXobj, marshal_as<String^>(strXml));
		//	}
		//}
	}
}
...
bool CWebRT::EclipseAppInit()
{
	Universe::WebRT::GetWebRT()->Fire_OnEclipseAppInit();
	return false;
}

bool CWebRT::OnUniversePreTranslateMessage(MSG* pMsg)
{
	return false;
};

void CWebRT::OnWebRTClose(CosmosCloseState state)
{
	switch (state)
	{
	case CosmosCloseState::StartClose:
	{
		AtlTrace(_T("*************Begin CWebRT::OnClose:  ****************\n"));
		...
		AtlTrace(_T("*************End CWebRT::OnClose:  ****************\n"));
	}
...
void CWebRT::OnObserveComplete(HWND hWnd, CString strUrl, IXobj* pRootXobj)
{
...
void CWebRTProxy::OnWebPageCreated(HWND hWnd, CWebPageImpl* pProxy, IWebPage* pChromeWebContent, int nState)
{
	if (nState == 0)
	{
