...
LRESULT CGridWnd::OnWebRTMsg(WPARAM wParam, LPARAM lParam)
{
	...
	case 20210202:
	{
		HWND hWnd = g_pWebRT->m_pUniverseAppProxy->QueryWndInfo(QueryType::RecalcLayout, m_hWnd);
		if (::IsWindow(hWnd))
		{
	...
	if (bAccept)
	{
		...
		else
		{
			...
			if (pClientObj)
			{
				...
				{
					g_pWebRT->ModifyBindingXobj(pBrowser->m_pParentXobj->m_pXobjShareData->m_pNucleus, pClientObj);
...
{
	cs.lpszClass = g_pWebRT->m_lpszSplitterClass;
...
int CGridWnd::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	...
	else if ((long)(g_pWebRT->m_pActiveAppProxy) > 1)
	{
