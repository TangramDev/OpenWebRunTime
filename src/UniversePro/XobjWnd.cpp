...
{
	m_pXobj = g_pWebRT->m_pActiveXobj;
	m_pXobj->m_nID = nID;
	...
	if (m_pXobj->m_strID.CompareNoCase(TGM_NUCLEUS) == 0)
	{
		...
		g_pWebRT->ModifyBindingXobj(pGalaxy, m_pXobj);
...
LRESULT CXobjWnd::OnAfterDPIChanged(WPARAM wParam, LPARAM lParam)
{
	if (m_pXobj->m_pWebBrowser)
	{
		g_pWebRT->m_mapDpiChangedBrowser[m_pXobj->m_pWebBrowser->m_hWnd] = m_pXobj->m_pWebBrowser;
	}
...
int CXobjWnd::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	...
	{
		::PostMessage(g_pWebRT->m_pActiveHtmlWnd->m_hWnd, WM_COSMOSMSG, 20190331, 0);
	...
	else if (g_pWebRT->m_pActiveAppProxy)
	{
	...
	{
		if (g_pWebRT->m_pNucleus != m_pXobj->m_pXobjShareData->m_pNucleus)
			::SetFocus(m_hWnd);
		g_pWebRT->m_pActiveXobj = m_pXobj;
	...
	else
	{
		if ((m_pXobj->m_nViewType != TabGrid && m_pXobj->m_nViewType != Grid))
		{
			if (g_pWebRT->m_pNucleus != m_pXobj->m_pXobjShareData->m_pNucleus || g_pWebRT->m_pActiveXobj != m_pXobj)
				::SetFocus(m_hWnd);
		}
	}
	g_pWebRT->m_pActiveXobj = m_pXobj;
	g_pWebRT->m_bWinFormActived = false;
	g_pWebRT->m_pNucleus = m_pXobj->m_pXobjShareData->m_pNucleus;

	CWebView* pHtmlWnd = g_pWebRT->m_pNucleus->m_pWebPageWnd;
...
BOOL CXobjWnd::PreTranslateMessage(MSG* pMsg)
{
		...
		else
		{
			if (m_pXobj->m_nViewType == CLRCtrl)
			{
				...
				if (::GetKeyState(VK_SHIFT) < 0)  // shift pressed
					bShiftKey = true;
				hr = g_pWebRT->m_pCLRProxy->ProcessCtrlMsg(::GetWindow(m_hWnd, GW_CHILD), bShiftKey ? true : false);
				if (hr == S_OK)
					return true;
				return false;
			}
	...
	else
	{
		if (m_pXobj->m_nViewType == CLRCtrl)
		{
			if (g_pWebRT->m_pCLRProxy->ProcessFormMsg(m_hWnd, pMsg, 0))
				return true;
			return false;
		}
	}
...
LRESULT CXobjWnd::OnTabChange(WPARAM wParam, LPARAM lParam)
{
	...
	if (pXobj)
	{
		...
		if (_pXobj->m_pWebBrowser) {
			g_pWebRT->m_pActiveHtmlWnd = _pXobj->m_pWebBrowser->m_pVisibleWebView;
		}
		if (lParam != wParam)
		{
	...
	if (lParam != wParam)
	{
		...
		if (pInfo)
		{
			...
			case 2:
			{
				...
				auto it = g_pWebRT->m_mapMDIParent.find(::GetParent(pInfo->m_hClient));
				if (it != g_pWebRT->m_mapMDIParent.end())
				{
					...
					if (pMDIClientObj)
					{
						g_pWebRT->ModifyBindingXobj(pMainParent->m_pNucleus, pMDIClientObj);
					}
				}
			}
			break;
		...
		if (pGalaxy->m_nGalaxyType != GalaxyType::CtrlBarGalaxy && pGalaxy->m_pWebPageWnd)
		{
			...
			{
				auto it = g_pWebRT->m_mapBrowserWnd.find(hWnd);
				if (it != g_pWebRT->m_mapBrowserWnd.end())
				{
					...
					g_pWebRT->m_mapSizingBrowser[hWnd] = pBrowser;
	...
	if (m_bCreateExternal)
	{
		...
		case WM_COMMAND:
		{
			...
			if (::IsWindow(hWnd) && m_pXobj)
			{
				...
				m_pXobj->Fire_ControlNotify(m_pXobj, HIWORD(wParam), LOWORD(wParam), lParam, CComBSTR(g_pWebRT->m_szBuffer));
				if (m_pXobj->m_bWebCommandMsg && m_pXobj->m_pWormhole)
				{
					...
					{
						::GetWindowText((HWND)lParam, g_pWebRT->m_szBuffer, MAX_PATH);
						strVal = g_pWebRT->m_szBuffer;
					}
...
LRESULT CBKWnd::OnMdiClientCreated(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL&)
{
	if (m_hChild)
	{
		...
		//g_pWebRT->m_mapBKFrame[m_hChild] = m_pNucleus;
	}
	return 1;
}
...
void CXobjWnd::OnWindowPosChanged(WINDOWPOS* lpwndpos)
{
	...
	if (bNotCtrlBar)
	{
		...
		if (pMainWnd)
		{
			CNucleus* pGalaxy = pMainWnd->m_pNucleus;
	...
	if (m_pXobj->m_pWebBrowser)
	{
		...
		if (lpwndpos->cx && lpwndpos->cy)
		{
			...
			else
			{
				...
				//UINT dpi = GetDpiForWindow(::GetAncestor(m_pXobj->m_pWebBrowser->m_hWnd,GA_ROOT));
				////FLOAT fscale = (float)dpi / USER_DEFAULT_SCREEN_DPI;
				////int iDpi = GetDpiForWindow(hWnd);
				//int dpiScaledX = MulDiv(-12, dpi, USER_DEFAULT_SCREEN_DPI);
				//int dpiScaledY = MulDiv(-6, dpi, USER_DEFAULT_SCREEN_DPI);
				//int dpiScaledWidth = MulDiv(lpwndpos->cx + 24, dpi, USER_DEFAULT_SCREEN_DPI);
				//int dpiScaledHeight = MulDiv(lpwndpos->cy + 18, dpi, USER_DEFAULT_SCREEN_DPI);
				//::SetWindowPos(m_pXobj->m_pWebBrowser->m_hWnd, HWND_TOP, dpiScaledX, dpiScaledY, dpiScaledWidth, dpiScaledHeight, SWP_NOACTIVATE | SWP_NOREDRAW | SWP_NOSENDCHANGING);
