...
STDMETHODIMP CEclipseExtender::get_ActiveWorkBenchWindow(BSTR bstrID, IWorkBenchWindow** pVal)
{
	...
	if (strID==_T(""))
	{
		if (g_pWebRT->m_pActiveEclipseWnd)
		{
			*pVal = (IWorkBenchWindow*)g_pWebRT->m_pActiveEclipseWnd;
			(*pVal)->AddRef();
		}
	}
	else
	{
		...
		g_pWebRT->get_RemoteCosmos(bstrID, &pWebRT);
		if (pWebRT)
		{
...
CEclipseWnd::CEclipseWnd(void)
{
	...
	m_pAppProxy = nullptr;
	m_pNuclei = nullptr;
	g_pWebRT->m_pActiveEclipseWnd = this;
}
...
void CEclipseWnd::OnFinalMessage(HWND hWnd)
{
	...
	if (it != g_pWebRT->m_mapWorkBenchWnd.end())
		g_pWebRT->m_mapWorkBenchWnd.erase(it);
	if (g_pWebRT->m_pCLRProxy)
	{
		g_pWebRT->m_pCLRProxy->ReleaseCosmosObj((IWorkBenchWindow*)this);
	}
...
void CEclipseWnd::CreatePage(BOOL bSaveToConfigFile)
{
	if (m_pNuclei == nullptr)
	{
		...
		g_pWebRT->CreateNucleusGroup((LONGLONG)m_hWnd, &pNuclei);
		if (pNuclei == nullptr)
			return;
		...
		if (hMenu)
		{
			...
			for (int i = nCount - 1; i >0; i--)
			{
				...
				mii.dwTypeData = g_pWebRT->m_szBuffer;
		...
		{
			m_strDocKey = g_pWebRT->m_strCurrentEclipsePagePath;
			g_pWebRT->m_strCurrentEclipsePagePath = _T("");
			Show(m_strDocKey);
			auto it2 = g_pWebRT->m_mapValInfo.find(_T("newmdtframe"));
			if (it2 != g_pWebRT->m_mapValInfo.end())
			{
				m_strFrameID = OLE2T(it2->second.bstrVal);
				g_pWebRT->m_mapValInfo.erase(it2);
			}
		}
	}
}
...
STDMETHODIMP CEclipseWnd::CloseTangramUI()
{
	if (m_pNucleus)
	{
		HWND _hWnd = ::CreateWindowEx(NULL, _T("Cosmos Xobj Class"), L"", WS_CHILD, 0, 0, 0, 0, g_pWebRT->m_hCosmosWnd, NULL, AfxGetInstanceHandle(), NULL);
...
void CEclipseWnd::Show(CString strID)
{
	...
	if (g_pWebRT->m_bIsEclipseInit == false)
		g_pWebRT->m_bIsEclipseInit = true;
	if (data==0&&g_pWebRT->m_pUniverseAppProxy)
	{
		...
		if (::IsChild(m_hWnd, m_hClient))
		{
			...
			{
				auto it = g_pWebRT->m_mapCreatingWorkBenchInfo.find(strID);
				if (it != g_pWebRT->m_mapCreatingWorkBenchInfo.end())
				{
					m_strDocKey = m_strXml = it->second;
					g_pWebRT->m_mapCreatingWorkBenchInfo.erase(it);
				}
				else
					m_strDocKey = m_strXml = m_strPath = g_pWebRT->m_strDefaultWorkBenchXml;
			}
			else
			{
			...
			if (m_strAppProxyID != _T(""))
			{
				...
				if (m_pAppProxy)
				{
					...
					::SetWindowText(m_hClient, g_pWebRT->m_strAppKey);
			...
			else if(strID !=_T(""))
			{
				...
				if (pGalaxy)
				{
					...
					if (m_strDocKey == _T(""))
					{
						m_strDocKey = m_strXml = m_strPath = g_pWebRT->m_strAppDataPath + _T("default.workbench");
					}
					else
					{
						strKey = strID;
						if (g_pWebRT->m_nAppType == APP_ECLIPSE)
							m_strDocKey = g_pWebRT->m_strStartupURL;
					}
...
{
	if (g_pWebRT->m_strWorkBenchStrs != _T(""))
	{
		int nPos = g_pWebRT->m_strWorkBenchStrs.Find(_T("|"));
		m_strDocKey = g_pWebRT->m_strWorkBenchStrs.Left(nPos);
		g_pWebRT->m_strWorkBenchStrs = g_pWebRT->m_strWorkBenchStrs.Mid(nPos + 1);
	}

	if (g_pWebRT->m_strCurrentEclipsePagePath != _T(""))
	{
		m_strDocKey = g_pWebRT->m_strCurrentEclipsePagePath;
		g_pWebRT->m_strCurrentEclipsePagePath = _T("");
	}
...
LRESULT CEclipseWnd::OnCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL&)
{
	if (wParam == m_nNewWinCmdID)
	{
		if (g_pWebRT->m_strCurrentEclipsePagePath == _T(""))
		{
			...
			g_pWebRT->m_strCurrentEclipsePagePath = OLE2T(bstrTemplate);
		}
		if (g_pWebRT->m_strCurrentEclipsePagePath == _T("")&& m_pNucleus)
		{
		}
	}
	LRESULT lRes = DefWindowProc(uMsg, wParam, lParam);
...
{
	m_strPath = g_pWebRT->m_strAppDataPath + _T("default.workbench");
	if (m_strPath != _T(""))
	{
...
LRESULT CEclipseWnd::OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& )
{
	...
	if (xmlParse.LoadFile(g_pWebRT->m_strConfigDataFile))
	{
		...
		if (pParse)
		{
			...
			xmlParse.SaveFile(g_pWebRT->m_strConfigDataFile);
		}
	}
	if (g_pWebRT->m_pActiveEclipseWnd == this)
		g_pWebRT->m_pActiveEclipseWnd = nullptr;
	auto it = g_pWebRT->m_mapWorkBenchWnd.find(m_hWnd);
	if (it != g_pWebRT->m_mapWorkBenchWnd.end())
	{
		g_pWebRT->m_mapWorkBenchWnd.erase(it);
	}
	LRESULT lRes = DefWindowProc(uMsg, wParam, lParam);
	if (g_pWebRT->m_mapWorkBenchWnd.size() == 0)
	{
