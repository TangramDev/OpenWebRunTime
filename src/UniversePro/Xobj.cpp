...
void CXobj::InitWndXobj()
{
	...
	else
	{
		...
		if (m_strID == _T(""))
		{
			...
			else if (m_pXobjShareData->m_pNucleus->m_pWebPageWnd)
			{
				auto it = m_pXobjShareData->m_pNucleus->m_pWebPageWnd->m_mapFormsInfo.find(m_strObjTypeID);
				if (it != m_pXobjShareData->m_pNucleus->m_pWebPageWnd->m_mapFormsInfo.end())
				{
					m_strID = _T("clrctrl");
					m_nViewType = CLRCtrl;
				}
	...
	{
		m_strNodeName = m_strName + _T("@") + g_pWebRT->m_strCurrentKey + _T("@") + m_pXobjShareData->m_pNucleus->m_strNucleusName;
	...
	{
		auto it = g_pWebRT->m_mapClassInfo.find(m_strID);
		if (it != g_pWebRT->m_mapClassInfo.end())
...
CXobj::~CXobj()
{
	if (g_pWebRT->m_pActiveXobj == this)
	...
	auto it = g_pWebRT->m_mapXobjMdiFormBindDic.find(this);
	if (it != g_pWebRT->m_mapXobjMdiFormBindDic.end())
	{
		g_pWebRT->m_mapXobjMdiFormBindDic.erase(it);
	}
...
CWebView* CXobj::GetHtmlWnd()
{
	if (m_pRootObj)
	{
		...
		if (::IsWindow(hWnd))
		{
			::GetClassName(hWnd, g_pWebRT->m_szBuffer, 256);
		...
		else
		{
			if (m_pParentWinFormWnd == nullptr)
			{
				...
				if (pForm)
				{
					...
					CXobj* pObj = (CXobj*)g_pWebRTImpl->GetXobj(hPWnd);
					if (pObj)
					{
...
STDMETHODIMP CXobj::ObserveEx(int nRow, int nCol, BSTR bstrKey, BSTR bstrXml, IXobj** ppRetXobj)
{
	...
	if (m_pXobjShareData->m_pNuclei && m_nViewType == Grid)
	{
		...
		if (pWndXobj->m_pHostGalaxy == nullptr)
		{
			...
			_pGalaxy->m_pWebPageWnd = m_pXobjShareData->m_pNucleus->m_pWebPageWnd;
			...
			if (m_pXobjShareData->m_pNucleus->m_pWebPageWnd)
			{
				_pGalaxy->m_pWebPageWnd = m_pXobjShareData->m_pNucleus->m_pWebPageWnd;
			}
		}
...
STDMETHODIMP CXobj::put_Attribute(BSTR bstrKey, BSTR bstrVal)
{
	if (m_pHostParse != nullptr)
	{
		...
		if (strVal.CompareNoCase(TGM_NUCLEUS) == 0)
		{
			...
			g_pWebRT->ModifyBindingXobj(pGalaxy, this);
...
BOOL CXobj::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext)
{
	...
	CWebView* pHtmlWnd = m_pXobjShareData->m_pNucleus->m_pWebPageWnd;
	HWND hWnd = 0;
	...
	if (m_strID == _T("activex") || m_strID == _T("clrctrl"))
	{
		...
		{
			CString strPath = g_pWebRT->m_strAppPath + _T("TangramWebPage\\") + m_strObjTypeID;
	...
	else
	{
		if (hWnd == NULL)
		{
			...
			if (nPos != -1)
			{
				...
				auto it = g_pWebRT->m_mapWebRTAppProxy.find(strKey);
				...
				else
				{
					...
					if (hHandle)
					{
						it = g_pWebRT->m_mapWebRTAppProxy.find(strKey.MakeLower());
						if (it != g_pWebRT->m_mapWebRTAppProxy.end())
						{
							pProxy = it->second;
						}
			...
			{
				IWebRTWindowProvider* pViewFactoryDisp = nullptr;
				auto it = g_pWebRT->m_mapWindowProvider.find(m_strObjTypeID);
				if (it != g_pWebRT->m_mapWindowProvider.end())
				{
				...
				else
				{
					if (it == g_pWebRT->m_mapWindowProvider.end())
					{
						if (m_strID.CompareNoCase(_T("TreeView")))
						{
							...
							{
								if (g_pWebRT->m_strExeName.CompareNoCase(_T("devenv")) == 0)
								{
#ifdef _WIN32
									CString strLib = g_pWebRT->m_strAppPath + _T("PublicAssemblies\\TabbedWnd.dll");
								...
								{
									CString strLib = g_pWebRT->m_strAppPath + _T("TabbedWnd.dll");
									if (!::PathFileExists(strLib))
									{
										strLib = g_pWebRT->m_strProgramFilePath + _T("\\tangram\\TabbedWnd.dll");
									}
									if (::PathFileExists(strLib))
									{
										::LoadLibrary(strLib);
							...
							{
								CString strLib = g_pWebRT->m_strAppPath + _T("mfctab.dll");
								if (::PathFileExists(strLib))
								{
									::LoadLibrary(strLib);
									auto it = g_pWebRT->m_mapWindowProvider.find(m_strObjTypeID);
							...
							{
								CString strLib = _T("");
								CString strPath = g_pWebRT->m_strAppPath + _T("wincomponent\\") + m_strObjTypeID + _T(".component");
								CTangramXmlParse m_Parse;
								if (::PathFileExists(strPath))
								{
									if (m_Parse.LoadFile(strPath))
									{
										strLib = g_pWebRT->m_strAppPath + _T("wincomponent\\") + m_Parse.attr(_T("lib"), _T(""));
									}
								}
								else
								{
									strPath = g_pWebRT->m_strProgramFilePath + _T("\\tangram\\wincomponent\\") + m_strObjTypeID + _T(".component");
									if (m_Parse.LoadFile(strPath))
									{
										strLib = g_pWebRT->m_strProgramFilePath + _T("\\tangram\\wincomponent\\") + m_Parse.attr(_T("lib"), _T(""));
									}
									else
									{
										strPath = g_pWebRT->m_strAppPath + _T("PublicAssemblies\\wincomponent\\") + m_strObjTypeID + _T(".component");
										if (m_Parse.LoadFile(strPath))
										{
											strLib = g_pWebRT->m_strAppPath + _T("PublicAssemblies\\wincomponent\\") + m_Parse.attr(_T("lib"), _T(""));
										}
									}
								}
				...
				if (pViewFactoryDisp)
				{
					...
					if (g_pWebRT->m_pCreatingWindow)
					{
						m_pWindow = g_pWebRT->m_pCreatingWindow;
						g_pWebRT->m_pCreatingWindow = nullptr;
					}
	...
	if (_strURL == _T("host"))
	{
		CBrowser* pBrowser = m_pXobjShareData->m_pNucleus->m_pHostWebBrowserWnd;
		if (pBrowser)
		{
		...
		else
		{
			...
			{
				hPWnd = g_pWebRT->m_hTempBrowserWnd;
			...
			else if (g_pWebRT->m_pHtmlWndCreated == nullptr)
			{
				hPWnd = g_pWebRT->m_hHostBrowserWnd;
			}
			else
			{
				hPWnd = ::GetParent(g_pWebRT->m_pHtmlWndCreated->m_hWnd);
			}
	...
	if (m_nViewType == BlankView && m_pHostParse != nullptr)
	{
		...
		else
		{
			if (m_pWebBrowser == nullptr && _strURL != _T(""))
			{
				...
				if (g_pWebRT->m_pBrowserFactory)
				{
					HWND hBrowser = g_pWebRT->m_pBrowserFactory->CreateBrowser(hWnd, s);
				...
				{
					g_pWebRT->m_mapXobjForHtml[this] = s;
				}
			}
		}
	}
	...
	if (m_strID.CompareNoCase(_T("TreeView")) == 0)
	{
		...
		if (strImages != _T(""))
		{
			...
			if (strImage != _T(""))
			{
				::GetTempPath(MAX_PATH, g_pWebRT->m_szBuffer);
				CString strPath = CString(g_pWebRT->m_szBuffer);
		...
		if (nCol)
		{
			...
			if (pTreeNodeParse)
			{
				...
				if (strTag.CompareNoCase(_T("tangramdesigner")) == 0)
				{
					...
					auto it = g_pWebRT->m_mapValInfo.find(strKey);
				...
				else
				{
					...
					if (bstrType != _T(""))
					{
						...
						{
							{ pWebRTDesignView->m_pXHtmlTree->m_hWnd, WM_HUBBLE_DESIGNERCMD, 1992, _T("WM_HUBBLE__DESIGNERCMD") },
							{ pWebRTDesignView->m_pXHtmlTree->m_hWnd, WM_HUBBLE_DESIGNERCMD, 1963, _T("WM_HUBBLE__DESIGNERCMD2") },
						};
	...
	if (g_pWebRT->m_pActiveXobj && g_pWebRT->m_pActiveXobj->m_pXobjShareData->m_pNuclei)
		g_pWebRT->m_pActiveXobj->m_pXobjShareData->m_pNuclei->Fire_NodeCreated(this);
...
void CXobj::NodeCreated()
{
	...
	if (m_pXobjShareData->m_pNucleus->m_pWebPageWnd == nullptr && pHtmlWnd)
		m_pXobjShareData->m_pNucleus->m_pWebPageWnd = pHtmlWnd;
	if (pHtmlWnd == nullptr)
		pHtmlWnd = m_pXobjShareData->m_pNucleus->m_pWebPageWnd;
	if (pHtmlWnd == nullptr)
		pHtmlWnd = g_pWebRT->m_pHostHtmlWnd;
...
HWND CXobj::CreateView(HWND hParentWnd, CString strTag)
{
	...
	{
		::GetClassName(_hWnd, g_pWebRT->m_szBuffer, 256);
		CString strName = CString(g_pWebRT->m_szBuffer);
		if (strName == _T("Chrome Extended Window Class")) {
	...
	case ActiveX:
	{
		...
		if (m_pDisp == nullptr)
		{
			...
			if (nPos != -1)
			{
				...
				if (dwProcessID)
				{
					auto it = g_pWebRT->m_mapRemoteTangramApp.find(dwProcessID);
					if (it != g_pWebRT->m_mapRemoteTangramApp.end())
					{
	...
	case CLRCtrl:
	{
		...
		if (g_pWebRT->m_pCLRProxy)
		{
			...
			{
				CWebPageImpl* pChromeRenderFrameHostProxyBase = nullptr;
				IWebPage* pChromeWebPage = nullptr;
				if (pHtmlWnd)
				{
					pChromeRenderFrameHostProxyBase = (CWebPageImpl*)pHtmlWnd;
					pChromeWebPage = (IWebPage*)pHtmlWnd;
					//m_pHostParse->put_attr(_T("webpage"), (__int64)pChromeWebPage);
					//m_pHostParse->put_attr(_T("webpagehandle"), (__int64)pHtmlWnd->m_hWnd);
					//CWebPageImpl* pChromeRenderFrameHostProxyBase = (CWebPageImpl*)pHtmlWnd;
					//m_pHostParse->put_attr(_T("renderframehostproxy"), (__int64)pChromeRenderFrameHostProxyBase);
					g_pWebRT->m_pCLRProxy->m_strCurrentWinFormTemplate = m_pHostParse->xml();
				}
				m_pDisp = g_pWebRT->m_pCLRProxy->CreateObject(strTag.AllocSysString(), hParentWnd, this, pChromeRenderFrameHostProxyBase, pChromeWebPage);
				if (g_pWebRT->m_hFormNodeWnd)
				{
					LRESULT l = ::SendMessage((HWND)g_pWebRT->m_hFormNodeWnd, WM_HUBBLE_DATA, 0, 20190214);
					if (l && pHtmlWnd)
					{
						auto it = pHtmlWnd->m_mapWinForm.find(g_pWebRT->m_hFormNodeWnd);
						if (it == pHtmlWnd->m_mapWinForm.end())
						{
							...
							auto it2 = g_pWebRT->m_mapBrowserWnd.find(hBrowser);
							if (it2 != g_pWebRT->m_mapBrowserWnd.end())
								pWnd->m_pBrowser = (CBrowser*)it2->second;
							pHtmlWnd->m_mapWinForm[g_pWebRT->m_hFormNodeWnd] = pWnd;
						}
					}
				}
				...
				{
					pWnd->m_hFormWnd = g_pWebRT->m_hFormNodeWnd;
	...
	if (m_pDisp)
	{
		if (m_nViewType == CLRCtrl)
		{
			if (g_pWebRT->m_hFormNodeWnd && (::GetWindowLongPtr(g_pWebRT->m_hFormNodeWnd, GWL_STYLE) & WS_CHILD))
			{
				HWND hCtrl = NULL;
				if (g_pWebRT->m_pCLRProxy)
					hCtrl = g_pWebRT->m_pCLRProxy->GetCtrlHandle(m_pDisp);
				if (hCtrl == g_pWebRT->m_hFormNodeWnd)
				{
					HWND hWnd = g_pWebRT->m_hFormNodeWnd;
...
{
	if (g_pWebRT->m_pCLRProxy && m_nViewType == CLRCtrl && m_pDisp)
		*ppRetDisp = g_pWebRT->m_pCLRProxy->GetCtrlByName(m_pDisp, bstrName, bFindInChild ? true : false);
...
STDMETHODIMP CXobj::GetCtrlValueByName(BSTR bstrName, VARIANT_BOOL bFindInChild, BSTR* bstrVal)
{
	if (g_pWebRT->m_pCLRProxy && m_nViewType == CLRCtrl && m_pDisp)
	{
		*bstrVal = g_pWebRT->m_pCLRProxy->GetCtrlValueByName(m_pDisp, bstrName, bFindInChild ? true : false);
	}
	return S_OK;
}
...
STDMETHODIMP CXobj::SetCtrlValueByName(BSTR bstrName, VARIANT_BOOL bFindInChild, BSTR bstrVal)
{
	if (g_pWebRT->m_pCLRProxy && m_nViewType == CLRCtrl && m_pDisp)
	{
		g_pWebRT->m_pCLRProxy->SetCtrlValueByName(m_pDisp, bstrName, bFindInChild ? true : false, bstrVal);
	}
	return S_OK;
}
...
{
	m_pXobjs = &m_vXobjs;
	g_pWebRT->m_mapWndXobjCollection[(__int64)this] = this;
}

CXobjCollection::~CXobjCollection()
{
	auto it = g_pWebRT->m_mapWndXobjCollection.find((__int64)this);
	if (it != g_pWebRT->m_mapWndXobjCollection.end())
	{
		g_pWebRT->m_mapWndXobjCollection.erase(it);
	}
	m_vXobjs.clear();
}
...
{
	g_pWebRT->UpdateXobj(m_pRootObj);
...
HRESULT CXobj::Fire_Destroy()
{
	...
	if (g_pWebRT->m_pCLRProxy)
	{
		g_pWebRT->m_pCLRProxy->ReleaseCosmosObj((IXobj*)this);
	}
	if (m_nViewType != TangramTreeView && m_nViewType != Grid && m_pDisp)
	{
		if (m_nViewType == CLRCtrl)
			g_pWebRT->m_pCLRProxy->ReleaseCosmosObj(m_pDisp);
		else
...
{
	if (g_pWebRT->m_pBrowserFactory == nullptr)
		return S_OK;
	if (m_pWebBrowser == nullptr)
	{
		...
		HWND hBrowser = g_pWebRT->m_pBrowserFactory->CreateBrowser(((CXobjWnd*)m_pHostWnd)->m_hWnd, s);
		((CXobjWnd*)m_pHostWnd)->m_hFormWnd = hBrowser;
		g_pWebRT->m_hParent = NULL;
		auto it = g_pWebRT->m_mapBrowserWnd.find(hBrowser);
		if (it != g_pWebRT->m_mapBrowserWnd.end())
		...
		//g_pWebRT->m_pCurWebNode = nullptr;
		return S_OK;
	}
	else if (m_pWebBrowser)
	{
...
STDMETHODIMP CXobj::put_URL(BSTR newVal)
{
	...
	if (m_pWebBrowser == nullptr)
	{
		...
		HWND hBrowser = g_pWebRT->m_pBrowserFactory->CreateBrowser(m_pHostWnd->m_hWnd, s);
		((CXobjWnd*)m_pHostWnd)->m_hFormWnd = hBrowser;
		g_pWebRT->m_hParent = NULL;
		auto it = g_pWebRT->m_mapBrowserWnd.find(hBrowser);
		if (it != g_pWebRT->m_mapBrowserWnd.end())
...
STDMETHODIMP CXobj::SendIPCMessage(BSTR bstrTo, BSTR bstrPayload, BSTR bstrExtra, BSTR bstrMsgId, BSTR* bstrRet)
{
	if (m_pXobjShareData->m_pNucleus->m_pWebPageWnd)
	{
		m_pXobjShareData->m_pNucleus->m_pWebPageWnd->SendChromeIPCMessage(_T("I_DONT_KNOW"), OLE2T(bstrTo), OLE2T(bstrMsgId), OLE2T(bstrExtra), L"", L"");
	}
	else
	{
		if (m_pXobjShareData->m_pNucleus->m_pHostWebBrowserNode)
		{
			if (m_pXobjShareData->m_pNucleus->m_pHostWebBrowserWnd)
			{
				...
				for (auto& it : g_pWebRT->m_mapWebView)
				{
