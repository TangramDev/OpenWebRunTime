...
#ifdef _DEBUG
	g_pWebRT->m_nTangramNodeCommonData++;
#endif	
};
...
CNuclei::~CNuclei()
{
	...
	if (g_pWebRT->m_mapNuclei.size() == 0)
		g_pWebRT->Close();
...
STDMETHODIMP CNuclei::CreateNucleus(VARIANT ParentObj, VARIANT HostWnd, BSTR bstrNucleusName, INucleus** pRetFrame)
{
	...
	if (ParentObj.vt == VT_DISPATCH && HostWnd.vt == VT_BSTR)
	{
		if (g_pWebRT->m_pCLRProxy)
		{
			...
			pDisp = g_pWebRT->m_pCLRProxy->GetCtrlByName(ParentObj.pdispVal, HostWnd.bstrVal, true);
			if (pDisp)
			{
				h = g_pWebRT->m_pCLRProxy->GetCtrlHandle(pDisp);
				if (h)
				{
	...
	if (HostWnd.vt == VT_DISPATCH)
	{
		if (g_pWebRT->m_pCLRProxy)
		{
			h = g_pWebRT->m_pCLRProxy->IsNucleus(HostWnd.pdispVal);
			if (h)
			{
				...
				{
					::SysFreeString(bstrName);
					bstrNucleusName = g_pWebRT->m_pCLRProxy->GetCtrlName(HostWnd.pdispVal);
	...
	else if (HostWnd.vt == VT_I2 || HostWnd.vt == VT_I4 || HostWnd.vt == VT_I8)
	{
		...
		if (_hWnd && ::IsWindow(_hWnd))
		{
			...
			if (it == m_mapGalaxy.end())
			{
				...
				::GetClassName(hPWnd, g_pWebRT->m_szBuffer, MAX_PATH);
				...
				else if (strClassName.Find(_T("Chrome Extended Window Class")) == 0)
				{
					...
					auto it = g_pWebRT->m_mapBrowserWnd.find(_hPWnd);
...
STDMETHODIMP CNuclei::GetNucleusFromCtrl(IDispatch* CtrlObj, INucleus** ppNucleus)
{
	if (g_pWebRT->m_pCLRProxy)
	{
		HWND h = g_pWebRT->m_pCLRProxy->IsNucleus(CtrlObj);
		if (h)
		{
...
void CNuclei::BeforeDestory()
{
	...
	if (g_pWebRT->m_pCLRProxy)
	{
		g_pWebRT->m_pCLRProxy->ReleaseCosmosObj((INuclei*)this);
	}
}
...
STDMETHODIMP CNuclei::get_Parent(INuclei** pVal)
{
	...
	while (it == g_pWebRT->m_mapNuclei.end())
	{
		hWnd = ::GetParent(hWnd);
		if (hWnd == NULL)
			return S_OK;
		it = g_pWebRT->m_mapNuclei.find(hWnd);
		if (it != g_pWebRT->m_mapNuclei.end())
		{
			*pVal = it->second;
			return S_OK;
...
STDMETHODIMP CNuclei::Observe(IDispatch* Parent, BSTR CtrlName, BSTR NucleusName, BSTR bstrKey, BSTR bstrXml, IXobj** ppRetXobj)
{
	...
	{
		IDispatch* pDisp = nullptr;
		pDisp = g_pWebRT->m_pCLRProxy->GetCtrlByName(Parent, CtrlName, true);
		if (pDisp)
		{
			HWND h = 0;
			h = g_pWebRT->m_pCLRProxy->IsNucleus(pDisp);
			if (h)
			{
...
STDMETHODIMP CNuclei::ObserveCtrl(VARIANT MdiForm, BSTR bstrKey, BSTR bstrXml, IXobj** ppRetXobj)
{
	...
	if (MdiForm.vt == VT_DISPATCH)
	{
		if (g_pWebRT->m_pCLRProxy)
		{
			h = g_pWebRT->m_pCLRProxy->GetMDIClientHandle(MdiForm.pdispVal);
			if (h)
				bMDI = true;
			else
			{
				h = g_pWebRT->m_pCLRProxy->IsNucleus(MdiForm.pdispVal);
				if (h)
				{
					CComBSTR bstrName(L"");
					bstrName = g_pWebRT->m_pCLRProxy->GetCtrlName(MdiForm.pdispVal);
					CString strKey = OLE2T(bstrKey);
					if (strKey == _T(""))
						bstrKey = CComBSTR(L"Default");
					INucleus* pGalaxy = nullptr;
