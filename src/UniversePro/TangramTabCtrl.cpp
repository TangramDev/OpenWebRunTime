...
CTangramListCtrl::CTangramListCtrl()
{
	...
	if (g_pWebRT->m_strExeName.CompareNoCase(_T("excel")) == 0)
	{
		auto it = g_pWebRT->m_mapValInfo.find(_T("exceldesignstate"));
		if (it == g_pWebRT->m_mapValInfo.end())
			g_pWebRT->m_mapValInfo[_T("exceldesignstate")] = CComVariant((VARIANT_BOOL)true);
	}
}
...
BOOL CTangramTabCtrl::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	...
	::GetModuleFileName(theApp.m_hInstance, g_pWebRT->m_szBuffer, MAX_PATH);
	...
	CString strPath = g_pWebRT->m_szBuffer;
...
LRESULT CTangramTabCtrl::OnInitialUpdate(WPARAM wParam, LPARAM lParam)
{
	...
	{
		if (g_pWebRT->m_DocTemplateImageList.m_hImageList == 0)
			g_pWebRT->m_DocTemplateImageList.Create(32, 32, ILC_COLOR32, 0, 4);
		m_ListCtrl.SetImageList(&g_pWebRT->m_DocTemplateImageList, LVSIL_NORMAL);
	}
...
int CTangramTabCtrl::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	...
	{
		HWND hMenuWnd = g_pWebRT->m_pUniverseAppProxy->GetActivePopupMenu(nullptr);
