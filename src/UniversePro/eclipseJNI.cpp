...
jstring tangram_extend(JNIEnv * env, jobject obj, jstring key, jstring data, jstring features) {
	...
	if (g_pWebRT)
	{
		...
		strRet = g_pWebRT->tangram_for_eclipse(strKey, strData, strFeatures);
	}
	if (_features != NULL) {
...
void registerNatives(JNIEnv *env) {
	...
	jclass bridge = env->FindClass(g_pWebRT->m_strBridgeJavaClass);
...
JavaResults * startJavaJNI(_TCHAR* libPath, _TCHAR* vmArgs[], _TCHAR* progArgs[], _TCHAR* jarFile)
{
	...
	for (i = 0; i < numVMArgs; i++) {
		...
		stroption = g_pWebRT->ConfigJavaVMInfo(stroption);
		if (stroption == _T(""))
	...
	init_args.version = g_pWebRT->m_nJVMVersion;
#endif
	...
	if (createJavaVM(&jvm, &env, &init_args) == 0)
	{
		...
		if (g_pWebRT)
		{
			g_pWebRT->m_pWebRTDelegate->m_pJVM = jvm;
			g_pWebRT->m_pWebRTDelegate->m_pJVMenv = env;
			if (env)
			{
				g_pWebRT->m_pWebRTDelegate->systemClass = env->FindClass("java/lang/System");
				try
				{
					if (g_pWebRT->m_pWebRTDelegate->systemClass != nullptr)
					{
						g_pWebRT->m_pWebRTDelegate->exitMethod = env->GetStaticMethodID(g_pWebRT->m_pWebRTDelegate->systemClass, "exit", "(I)V");
						g_pWebRT->m_pWebRTDelegate->loadMethod = env->GetStaticMethodID(g_pWebRT->m_pWebRTDelegate->systemClass, "load", "(Ljava/lang/String;)V");
					}
				}
				catch (...)
				{
		...
		registerNatives(env);
		USES_CONVERSION;
		char * mainClassName = W2A(g_pWebRT->InitEclipse(jarFile));
