...
#ifdef _DEBUG
	g_pWebRT->m_nSessionCount++;
#endif	
}

CWormhole::~CWormhole()
{
	auto it = g_pWebRT->m_mapWormhole.find(m_pSession);
	if (it != g_pWebRT->m_mapWormhole.end())
	{
		g_pWebRT->m_mapWormhole.erase(it);
	}
#ifdef _DEBUG
	g_pWebRT->m_nSessionCount--;
#endif	
...
void CWormhole::SendMessage()
{
	if (m_pOwner)
	{
		...
		//	auto it = g_pWebRT->m_mapBrowserWnd.find(::GetParent(pView->m_hWnd));
