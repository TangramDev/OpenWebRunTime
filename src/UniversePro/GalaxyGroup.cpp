 ...
 *******************************************************************************/
 ...
 // GalaxyGroup.cpp : Implementation of CGalaxyGroup
...
STDMETHODIMP CGalaxyGroup::get_Count(long* pCount)
{
	*pCount = (long)m_mapGalaxy.size();
	return S_OK;
}

STDMETHODIMP CGalaxyGroup::get_WinNucleus(VARIANT vIndex, IWinNucleus** ppGalaxy)
{
	if (vIndex.vt == VT_I4)
	{
...
STDMETHODIMP CGalaxyGroup::get__NewEnum(IUnknown** ppVal)
{
...
STDMETHODIMP CGalaxyGroup::CreateNucleus(VARIANT ParentObj, VARIANT HostWnd, BSTR bstrNucleusName, IWinNucleus** pRetFrame)
{
	HWND h = 0;
	...
	if (HostWnd.vt == VT_DISPATCH)
	{
		if (g_pCosmos->m_pCLRProxy)
		{
			...
			if (h)
			{
				...
				{
					::SysFreeString(bstrName);
					bstrNucleusName = g_pCosmos->m_pCLRProxy->GetCtrlName(HostWnd.pdispVal);
	...
	else if (HostWnd.vt == VT_I2 || HostWnd.vt == VT_I4 || HostWnd.vt == VT_I8)
	{
		...
		if (_hWnd && ::IsWindow(_hWnd))
		{
			...
			if (it == m_mapGalaxy.end())
			{
				...
				CWinNucleus* _pGalaxy = new CComObject<CWinNucleus>();
				CString strName = strNucleusName;
				if (strName == _T(""))
					strName = _T("default");
				strName.MakeLower();
...
STDMETHODIMP CGalaxyGroup::GetNucleusFromCtrl(IDispatch* CtrlObj, IWinNucleus** ppGalaxy)
{
	if (g_pCosmos->m_pCLRProxy)
	{
...
STDMETHODIMP CGalaxyGroup::GetXobj(BSTR bstrNucleusName, BSTR bstrNodeName, IXobj** pRetNode)
{
...
STDMETHODIMP CGalaxyGroup::get_NucleusName(LONGLONG hHwnd, BSTR* pVal)
{
...
void CGalaxyGroup::BeforeDestory()
{
	Fire_Destroy();
	...
	if (g_pCosmos->m_pCLRProxy)
	{
		g_pCosmos->m_pCLRProxy->ReleaseCosmosObj((IGalaxyGroup*)this);
	}
}

STDMETHODIMP CGalaxyGroup::get_Xobj(BSTR bstrNodeName, IXobj** pVal)
{
	CString strName = OLE2T(bstrNodeName);
	if (strName == _T(""))
		return S_OK;
...
STDMETHODIMP CGalaxyGroup::get_XObject(BSTR bstrName, IDispatch** pVal)
{
...
STDMETHODIMP CGalaxyGroup::get_Width(long* pVal)
{
	if (m_hWnd)
	{
		RECT rc;
...
STDMETHODIMP CGalaxyGroup::put_Width(long newVal)
{
	if (m_hWnd && newVal)
	{
		RECT rc;
...
STDMETHODIMP CGalaxyGroup::get_Height(long* pVal)
{
	if (m_hWnd)
	{
		RECT rc;
...
STDMETHODIMP CGalaxyGroup::put_Height(long newVal)
{
	if (m_hWnd && newVal)
	{
		RECT rc;
...
STDMETHODIMP CGalaxyGroup::get_XobjNames(BSTR* pVal)
{
	CString strNames = _T("");
...
STDMETHODIMP CGalaxyGroup::get_Parent(IGalaxyGroup** pVal)
{
...
STDMETHODIMP CGalaxyGroup::get_Handle(LONGLONG* pVal)
{
	if (m_hWnd)
...
STDMETHODIMP CGalaxyGroup::GetCtrlInXobj(BSTR NodeName, BSTR CtrlName, IDispatch** ppCtrl)
{
...
STDMETHODIMP CGalaxyGroup::Observe(IDispatch* Parent, BSTR CtrlName, BSTR NucleusName, BSTR bstrKey, BSTR bstrXml, IXobj** ppRetXobj)
{
	if (g_pCosmos->m_pCLRProxy)
	{
		IDispatch* pDisp = nullptr;
...
STDMETHODIMP CGalaxyGroup::ObserveCtrl(VARIANT MdiForm, BSTR bstrKey, BSTR bstrXml, IXobj** ppRetXobj)
{
	HWND h = 0;
	bool bMDI = false;
			...
			else
			{
				...
				if (h)
				{
					...
					if (strKey == _T(""))
						bstrKey = CComBSTR(L"Default");
					IWinNucleus* pGalaxy = nullptr;
					map<HWND, CWinNucleus*>::iterator it = m_mapGalaxy.find((HWND)h);
					if (it == m_mapGalaxy.end())
					{
...
HRESULT CGalaxyGroup::Fire_GalaxyClusterLoaded(IDispatch* sender, BSTR url)
{
	HRESULT hr = S_OK;
...
HRESULT CGalaxyGroup::Fire_NodeCreated(IXobj* pXobjCreated)
{
	HRESULT hr = S_OK;
	int cConnections = m_vec.GetSize();
	if (cConnections)
...
HRESULT CGalaxyGroup::Fire_AddInCreated(IXobj* pRootXobj, IDispatch* pAddIn, BSTR bstrID, BSTR bstrAddInXml)
{
	HRESULT hr = S_OK;
	int cConnections = m_vec.GetSize();
	if (cConnections)
...
HRESULT CGalaxyGroup::Fire_BeforeOpenXml(BSTR bstrXml, LONGLONG hWnd)
{
	HRESULT hr = S_OK;
	int cConnections = m_vec.GetSize();
	if (cConnections)
...
HRESULT CGalaxyGroup::Fire_OpenXmlComplete(BSTR bstrXml, LONGLONG hWnd, IXobj* pRetRootNode)
{
	HRESULT hr = S_OK;
	int cConnections = m_vec.GetSize();
	if (cConnections)
...
HRESULT CGalaxyGroup::Fire_Destroy()
{
	HRESULT hr = S_OK;
	int cConnections = m_vec.GetSize();
	if (cConnections)
...
HRESULT CGalaxyGroup::Fire_NodeMouseActivate(IXobj* pActiveNode)
{
	HRESULT hr = S_OK;
	int cConnections = m_vec.GetSize();
	if (cConnections)
...
HRESULT CGalaxyGroup::Fire_ClrControlCreated(IXobj* Node, IDispatch* Ctrl, BSTR CtrlName, LONGLONG CtrlHandle)
{
	HRESULT hr = S_OK;
	int cConnections = m_vec.GetSize();
	if (cConnections)
...
HRESULT CGalaxyGroup::Fire_IPCMsg(IWinNucleus* pSender, BSTR bstrType, BSTR bstrContent, BSTR bstrFeature)
{
	HRESULT hr = S_OK;
	int cConnections = m_vec.GetSize();
	if (cConnections)
...
HRESULT CGalaxyGroup::Fire_TabChange(IXobj* sender, LONG ActivePage, LONG OldPage)
{
	HRESULT hr = S_OK;
	int cConnections = m_vec.GetSize();
	if (cConnections)
...
HRESULT CGalaxyGroup::Fire_CosmosEvent(ICosmosEventObj* pEventObj)
{
...
STDMETHODIMP CGalaxyGroup::get_GalaxyGroupXML(BSTR* pVal)
{
...
STDMETHODIMP CGalaxyGroup::ObserveNuclei(BSTR bstrGalaxys, BSTR bstrKey, BSTR bstrXml, VARIANT_BOOL bSaveToConfigFile)
{
