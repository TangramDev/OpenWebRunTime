...
BOOL CWPFView::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext)
{
	...
	g_pWebRT->LoadCLR();
	m_pWebRTWPFObj = g_pWebRT->m_pCLRProxy->CreateWPFControl(m_pXobj, pParentWnd->m_hWnd, nID);
...
int CWPFView::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	...
	g_pWebRT->m_pNucleus->HostPosChanged();
	g_pWebRT->m_pNucleus->UpdateVisualWPFMap(::GetParent(m_hWnd),false);
...
void CWPFView::OnDestroy()
{
	...
	g_pWebRT->m_pCLRProxy->WindowDestroy(m_hWnd);
	CView::OnDestroy();
}
